...
from se_theory_identity_regimes.lean_surface import SURFACE_BY_KIND
from se_theory_identity_regimes.load import load_json, load_reference_index, load_toml
from se_theory_identity_regimes.paths import reference_artifact_path, repo_root
...
_SURFACE_KIND_TO_ARTIFACT_ID: dict[str, str] = {
    "predicate": "regime-predicates",
    "theorem": "regime-theorems",
    "type": "regime-types",
...
def validate_reference_index(index: ReferenceIndex) -> list[str]:
    ...
    repo = index.get("repo")
    if repo != "se-theory-identity-regimes":
        errors.append(
            'reference/index.toml must define repo = "se-theory-identity-regimes"'
        )
