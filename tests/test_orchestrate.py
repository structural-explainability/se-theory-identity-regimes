...
def mock_sync_all() -> Generator[MagicMock]:
    with patch("se_theory_identity_regimes.orchestrate.sync_all") as m:
        yield m
...
def mock_loads() -> Generator[None]:
    ...
    with (
        patch(
            "se_theory_identity_regimes.orchestrate.load_manifest",
            return_value=manifest,
        ),
        patch(
            "se_theory_identity_regimes.orchestrate.load_manifest_schema",
            return_value=schema,
        ),
        patch(
        ...
        patch(
            "se_theory_identity_regimes.orchestrate.validate_manifest", return_value=[]
        ),
    ):
        yield
...
def test_run_validate_require_tag_calls_validate_tag() -> None:
    ...
    with (
        ...
        patch(
            "se_theory_identity_regimes.orchestrate.validate_tag", return_value=[]
        ) as mock_tag,
    ):
